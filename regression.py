...
#df = df.drop(['Number of Collections','Tonnes Collected','Number of Collections','Number of Collections','Tonnes Collected','Number of Collections','Tonnes Collected'],axis=1) # Drop Adj close and volume feature
df_train = df[:30]; # 60% training data and 40% testing data
df_test = df[30:]; 
scaler = MinMaxScaler() # For normalizing dataset
# We want to predict Close value of stock 
X_train = scaler.fit_transform(df_train.drop(['Rubbish Tonnes Collected'],axis=1).as_matrix())
y_train = scaler.fit_transform(df_train['Rubbish Tonnes Collected'].reshape(-1,1))



# y is output and x is features.
X_test = scaler.fit_transform(df_test.drop(['Rubbish Tonnes Collected'],axis=1).as_matrix())

y_test = scaler.fit_transform(df_test['Rubbish Tonnes Collected'].reshape(-1,1)); 

def denormalize(df,norm_data):
    df = df['Rubbish Tonnes Collected'].values.reshape(-1,1)
...
with tf.Session() as sess:
    ...
    plt.legend(loc='best')
    plt.ylabel('Rubbish tonnes collected')
    plt.xlabel('Days')
    plt.title('Waste Generated')
    plt.show()
    '''
...
rms = sqrt(mean_squared_error(df_train, df_test))
