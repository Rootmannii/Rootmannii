...
df = pd.read_csv('california_housing_train.csv') # read data set using pandas
print(df.info()) # Overview of dataset



df = df.drop(['longitude'],axis=1) # Drop Date feature
df = df.dropna(inplace=False)  # Remove all nan entries.

df = df.drop(['median_income', 'median_house_value'],axis=1) # Drop Adj close and volume feature
df_train = df[:200]; # 60% training data and 40% testing data
df_test = df[200:]; 
...
y_train = scaler.fit_transform(df_train['Close'].reshape(-1,1))
...
y_test = scaler.fit_transform(df_test['Close'].reshape(-1,1)); 
